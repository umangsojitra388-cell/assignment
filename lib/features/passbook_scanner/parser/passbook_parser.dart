...
class PassbookParser {
  static final _ifscSpaced = RegExp(
    r'\b([A-Za-z]{4})\s*0\s*([A-Za-z0-9]{6})\b',
  );
  static final _ifscCompact = RegExp(
  ...
  BankDetails parse(String rawText) {
    final normalized = TextCleaner.normalize(rawText);
  ...
  String? _extractIfsc(List<String> lines) {
    for (final line in lines) {
      ...
      final probe =
          RegExp(r'\d').hasMatch(line) ? TextCleaner.fixDigitConfusion(upper) : upper;
      final match = _ifscSpaced.firstMatch(probe) ?? _ifscCompact.firstMatch(probe);
      if (match == null) {
        continue;
      }
      final bank = match.group(1)!.toUpperCase();
      final branch =
          TextCleaner.fixDigitConfusion(match.group(2)!.toUpperCase());
      final composed = '${bank}0$branch';
      if (composed.length == 11) {
        return composed;
      }
    }
    return null;
  ...
  String? _extractAccountNumber(List<String> lines, String? ifsc) {
    ...
    for (final line in lines) {
      ...
      if (_ifscSpaced.hasMatch(upperLine) || _ifscCompact.hasMatch(upperLine)) {
        continue;
      }

      final hasKeyword = _accountKeyword.hasMatch(line);
      final digitLine = RegExp(r'\d').hasMatch(line)
          ? TextCleaner.fixDigitConfusion(line)
          : line;
    ...
    for (final line in lines) {
      final upperLine = line.toUpperCase();
      if (_ifscSpaced.hasMatch(upperLine) ||
          _ifscCompact.hasMatch(upperLine) ||
          _accountKeyword.hasMatch(line)) {
        continue;
      }

      final digitLine =
          RegExp(r'\d').hasMatch(line) ? TextCleaner.fixDigitConfusion(line) : line;
      for (final match in _digitRuns.allMatches(digitLine)) {
  ...
  bool _isValidAccountNumber(String digits, String? ifsc) {
    ...
    if (ifsc != null) {
      final compactIfsc = ifsc.replaceAll(RegExp(r'\s'), '');
      if (compactIfsc.contains(digits)) {
        return false;
      }
    }
    return true;
